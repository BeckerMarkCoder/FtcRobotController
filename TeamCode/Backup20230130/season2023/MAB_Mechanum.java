...
@TeleOp(name = "Basic: MAB_Mechanum ", group = "Iterative Opmode")
@Disabled
public class MAB_Mechanum extends OpMode {
    ...
    public void loop() {
        //double lHue = 0.0;
        //lHue = getLeftHue();
        //telemetry.addData("Left Hue:", String.format("%d", lHue));
        CheckGamePad2();
        CheckGamePad1();
    ...
    //Bumpers:    
        if (gamepad2.left_bumper) {
            leftpickup.setPower(intakespeed);
            rightpickup.setPower(intakespeed);
        }
        else{
            leftpickup.setPower(0.0);
            rightpickup.setPower(0.0);
        }
    ...
    private void CheckGamePad1() {
        ...
        y = Math.pow(gamepad1.left_stick_y * -1, 3);
    //DPad    
        if(gamepad1.dpad_down)  { y -= dpadspeed; }
        if(gamepad1.dpad_up)    { y += dpadspeed; }
        if(gamepad1.dpad_right) { x += dpadspeed; }
        if(gamepad1.dpad_left)  { x -= dpadspeed; }

        //if (EnableFieldCentric) {
        //    Radians = Math.toRadians(currentAngle * -1);
        //    x = x * Math.cos(Radians) - y * Math.sin(Radians);
        //    y = x * Math.sin(Radians) + y * Math.cos(Radians);
        //}
        
        //FLMPos = frontleftmotor.getCurrentPosition();
        //telemetry.addData("frontleftmotor", "%.2f", FLMPos);
        ...
        frontleftpower = x + y + rotation;
        frontrightpower = - x + y - rotation;
        backleftpower = - x + y + rotation;
        backrightpower = x + y - rotation;
